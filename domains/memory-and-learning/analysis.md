...
| R85 | 2026-02-17 | neural-network-impl callbacks system — trait design vs invocation | 1 | **Callbacks trait NOT INVOKED in training loop** — EarlyStoppingCallback and CheckpointCallback both defined with correct state tracking and test coverage (92-95%), but training loop in mod.rs (lines 206-215) reimplements early stopping logic directly instead of calling callback trait methods. Callbacks instantiated but never used (dead code). CheckpointCallback.on_epoch_end() is logging-only stub (zero I/O). Confirms R23 qualification gap: integration wrapper theatrical even in BEST_IN_ECOSYSTEM crate. 3 findings: C163 (trait not invoked), C164 (checkpoint stub), C165 (EarlyStopping genuine but duplicate). DEEP: 469 (from 468). |

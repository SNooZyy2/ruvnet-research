#!/bin/bash
# ruv-FANN compilation audit — sequential, crash-safe, portable
# Has multiple sub-workspaces so we need to cd into the right one per crate
#
# ENV VARS:
#   RUVFANN_REPO   — path to ruv-FANN repo clone
...
RESULTS="$RESEARCH/rust-build-tests/results-ruv-fann.txt"
ERRORS="$RESEARCH/rust-build-tests/results-ruv-fann-errors.txt"
BATCH_FILE="${1:-$RESEARCH/rust-build-tests/batches/ruv-fann-all.txt}"
DONE_FILE="$RESEARCH/rust-build-tests/ruv-fann-done.txt"
...
if [ ! -d "$REPO" ]; then
    echo "ERROR: ruv-FANN repo not found at $REPO"
    echo "Set RUVFANN_REPO env var or clone to ~/repos/ruv-FANN"
    exit 1
fi
...
get_workspace_dir() {
    ...
    case "$crate" in
        ruv-fann) echo "$REPO" ;;
        ruv-swarm-*|claude-parser|swe-bench-adapter|ml-training|benchmarking) echo "$REPO/ruv-swarm" ;;
        neuro-divergent*) echo "$REPO/neuro-divergent" ;;
        opencv-*) echo "$REPO/opencv-rust" ;;
        cuda-wasm) echo "$REPO/cuda-wasm" ;;
        *) echo "$REPO" ;;
    esac
}
...
while IFS= read -r line; do
    ...
    crate_dir=$(find . -maxdepth 4 -name Cargo.toml -not -path '*/target/*' \
        -exec grep -l "name = \"$crate\"" {} \; 2>/dev/null | head -1 | xargs dirname 2>/dev/null || true)
    if [ -n "$crate_dir" ] && [ -d "$crate_dir/src" ]; then

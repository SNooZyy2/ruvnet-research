...
Post-proposal code review identified 8 critical/high bugs and 4 minor gaps. All fixes applied inline below. Key changes:

1. **CRITICAL: ID namespace mismatch fixed** — `semantic-search.js` now strips `f-` prefix instead of bare `parseInt()` (was returning zero results)
2. **CRITICAL: VectorDb pollution fixed** — dedup and priority scripts now isolate temp indexes via CWD change + cleanup
3. **Phase 1 now deletes stale `ruvector.db`** before population
4. **CWD guard added** to all scripts
5. **`insertBatch()` used** in embed script (was single-insert loop)
6. **Dead schema columns removed** — `centroid` and `distance` were never computed
7. **Per-embedding error handling added** — individual failures no longer roll back entire batch
8. **Open Question #2 answered** — HNSW defaults M=16, efConstruction=200 confirmed adequate
9. **Concurrent write risk** added to Risks table
10. **Severity prepend documented** as intentional design choice
11. **`delete()` behavior documented** in API reference (soft-delete, works on JS binding)
12. **Description length claim corrected** — max 903 chars (~130 words), truncation risk acknowledged
...
| Quality | Good for short text similarity; sufficient for finding descriptions (avg ~30 words, max ~130 words). Findings >256 tokens (~⅓ of corpus) will be silently truncated — acceptable quality loss for semantic similarity |

**Embedding text format**: Finding text is embedded as `"{severity} {category}: {description}"`. This is a deliberate design choice — it means two identical descriptions with different severities will have slightly different embeddings. This improves dedup accuracy (CRITICAL and INFO findings about the same issue should cluster separately) but means pure semantic search may rank CRITICAL findings slightly higher by proximity even when searching for a concept, not a severity level.
...
async function main() {
  ...
  // CWD guard — must run from project root for correct ruvector.db
  const PROJECT_ROOT = '/home/snoozyy/ruvnet-research';
  if (!require('fs').existsSync('./db/research.db')) {
    process.chdir(PROJECT_ROOT);
    if (!require('fs').existsSync('./db/research.db'))
      throw new Error('Must run from ' + PROJECT_ROOT);
  }
  ...
  for (const r of results) {
    // IDs are stored as 'f-{finding_id}' — strip prefix before lookup
    const idStr = String(r.id).replace(/^f-/, '');
    const findingId = parseInt(idStr);
...
async function main() {
  ...
  // IMPORTANT: VectorDb auto-persists to ruvector.db in CWD.
  // We chdir to /tmp to avoid polluting the project's ruvector.db.
  const originalCwd = process.cwd();
  const tmpDir = require('os').tmpdir() + '/ruvector-dedup-' + Date.now();
  ...
  // Cleanup: restore CWD and remove temp VectorDb
  ...
  if (persist && clusters.length > 0) {
    ...
    const insertMember = sqlite.prepare(
      'INSERT INTO semantic_cluster_members (cluster_id, finding_id) VALUES (?, ?)'
    );
    ...
    const persistAll = sqlite.transaction(() => {
      for (const [i, cluster] of clusters.entries()) {
        ...
        for (const fid of cluster) {
          insertMember.run(clusterId, fid);
        }
      }
    });
    persistAll();
...
async function main() {
  ...
  // 1. Build index of DEEP file finding embeddings in temp dir
  //    (avoid polluting project's ruvector.db with non-finding vectors)
  const originalCwd = process.cwd();
  const tmpDir = require('os').tmpdir() + '/ruvector-priority-' + Date.now();
  require('fs').mkdirSync(tmpDir, { recursive: true });
  process.chdir(tmpDir);
...
async function main() {
  ...
  for (let i = 0; i < findings.length; i += txSize) {
    ...
    // Embed sequentially (one text at a time) with per-item error handling
    const embedded = [];
    const vdbBatch = [];
    for (let j = 0; j < texts.length; j++) {
      try {
        ...
        // Use 'f-{id}' prefix to namespace finding vectors in persistent VectorDb
        vdbBatch.push({ id: `f-${batch[j].id}`, vector: vec });
      } catch (err) {
        console.warn(`  SKIP finding ${batch[j].id}: ${err.message}`);
      }
    }

    // Batch-insert into persistent VectorDb (faster than single inserts)
    if (vdbBatch.length > 0) await vdb.insertBatch(vdbBatch);
...
1. **Delete or rename existing `ruvector.db`** — it contains 7 stale test vectors (batch-1, batch-2, test1, test2, v1, v2, v3) that would pollute the index
2. Create the 3 new tables (`embedding_registry`, `semantic_clusters`, `semantic_cluster_members`) in research.db
3. **Update `db/schema.sql`** to include the new tables and indexes (keep schema documentation in sync per ADR-038)
4. Run `scripts/embed-findings.js` to populate both `embedding_registry` and VectorDb (~4 min)
5. Verify: `SELECT COUNT(*) FROM embedding_registry WHERE source_table = 'findings'` should equal findings count
6. Spot-check: run `semantic-search.js` with a known query, manually verify top results make sense
...
| Concurrent VectorDb writes corrupt index | Medium | `ruvector.db` (hnswlib) has no write locking. Run `embed-findings.js` from ONE agent only — never in parallel. SQLite WAL mode is safe for concurrent reads |
| Finding text exceeds 256-token limit | Low | ~⅓ of findings have >200 chars. Truncation is silent but acceptable — semantic similarity degrades gracefully |
...
// Delete — works on the JS binding (confirmed empirically).
// NOTE: This is a soft-delete — the vector is marked removed in the HNSW
// graph but memory is not reclaimed. The Rust crate has known HNSW deletion
// issues, but the JS binding's soft-delete behavior is functional.
...
2. ~~**efSearch calibration**~~ **ANSWERED**: The JS binding inherits `hnsw_rs` defaults: M=16, efConstruction=200. Research sessions R12 and R98 confirmed these parameters through detailed HNSW analysis of the Rust crate. At 12K vectors, these defaults are more than adequate — M=16 provides good recall/speed tradeoff up to ~100K vectors, and efConstruction=200 ensures high-quality graph construction. No tuning needed
